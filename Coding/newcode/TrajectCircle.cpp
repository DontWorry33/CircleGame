...
Game::Game() : mBackgroundTexture(), mBackground(),
			   ...
			   mIsMovingLeft(false), mIsSpaceBar(false), mIsTab(false), mStatisticsText(), mStatisticsUpdateTime(),
...
void Game::update(sf::Time elapsedTime, Entity* entities[ENTITIES_MAX])
{
	...
	//currentlySelected, allows for cycling between Bread, using Tab. Before finally choosing, using SpaceBar.
